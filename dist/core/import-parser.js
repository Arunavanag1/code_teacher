...
import { createGraph, getCentrality, getEntryPoints, } from './dependency-graph.js';
// ---------------------------------------------------------------------------
 ...
 */
function resolveSpecifier(specifier, sourceFile, relativePathSet, projectPath) {
    ...
    // Try exact match
    if (relativePathSet.has(relBase))
        return relBase;
    // Try with common extensions
    for (const ext of RESOLVE_EXTENSIONS) {
        const withExt = relBase + ext;
        if (relativePathSet.has(withExt))
            return withExt;
    }
    // Try as directory with index file
    for (const ext of RESOLVE_EXTENSIONS) {
        const indexPath = relBase + '/index' + ext;
        if (relativePathSet.has(indexPath))
            return indexPath;
    }
    ...
    if (specifier.endsWith('.js')) {
        ...
        if (relativePathSet.has(tsBase))
            return tsBase;
        const tsxBase = relative(projectPath, resolve(projectPath, sourceDir, specifier.slice(0, -3) + '.tsx'));
        if (relativePathSet.has(tsxBase))
            return tsxBase;
    }
    return undefined;
}
...
function extractExportedSymbols(content, ext) {
    ...
    if (['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs'].includes(ext)) {
        ...
        const namedRe = /export\s+(?:default\s+)?(?:function|const|let|var|class|interface|type|enum)\s+(\w+)/g;
        let m;
 ...
 */
export async function parseImports(files, projectPath) {
...
// Helpers
// ---------------------------------------------------------------------------
function inferLanguageFromExt(ext) {
    const langMap = {
        '.ts': 'TypeScript',
        '.tsx': 'TypeScript',
        '.js': 'JavaScript',
 ...
 */
function computeNodeDepth(graph, nodeId) {
