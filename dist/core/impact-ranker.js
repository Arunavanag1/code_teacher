 ...
 */
function fanInToBlastRadius(fanIn) {
    if (fanIn === 0)
        return 0;
    if (fanIn <= 2)
        return Math.round(1 + fanIn);
    if (fanIn <= 5)
        return Math.round(2 + fanIn * 0.6);
    if (fanIn <= 7)
        return Math.round(fanIn * 0.85);
    if (fanIn <= 10)
        return Math.min(9, Math.round(fanIn * 0.9));
    return 10;
}
/**
 * Derives refactor risk from fan-in, coupling depth, and structural significance.
 */
function computeRefactorRisk(fanIn, couplingDepth, significance) {
 ...
 */
function computeKnowledgeGate(fanIn, centrality) {
 ...
 */
export function computeImpactRanking(stage1Results) {
    ...
    for (const id of nodeMap.keys())
        allFiles.add(id);
    for (const file of teachScoreMap.keys())
        allFiles.add(file);
    for (const file of significanceMap.keys())
        allFiles.add(file);
    ...
    for (const file of allFiles) {
        ...
        const compositeScore = Math.round((blastRadius * 0.3 +
            knowledgeGate * 0.25 +
            refactorRisk * 0.25 +
        ...
        const parts = [];
        if (fanIn > 3)
            parts.push(`${fanIn} dependents`);
        if (centrality > 5)
            parts.push(`high centrality (${centrality})`);
        if (significance > 5)
            parts.push(`significant structural decisions`);
        if (teachScore > 6)
            parts.push(`high educational value`);
