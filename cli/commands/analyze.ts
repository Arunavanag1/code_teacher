...
export async function analyzeCommand(path: string, options: AnalyzeOptions): Promise<void> {
  ...
  const { graph: depGraph, agentResult: depMapperResult } = await parseImports(
    files,
    resolved.targetPath,
  );
  ...
  const prioritizedChunks = new Map<string, Chunk[]>();
  ...
  const stage1Results: AgentResult[] = [depMapperResult, ...expandedLlmResults];
